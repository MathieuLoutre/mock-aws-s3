...
class S3Mock {
    ...
    listObjects(search, callback) {
        ...
        if (truncated && search.Delimiter) {
            // @ts-expect-error
            result.NextMarker = underscore_1.default.last(result.Contents).Key;
        }
        ...
        if (!callback) {
            ...
            fs_extra_1.default.readFile(search.Bucket + '/' + search.Key, function (err, data) {
                if (!err) {
                    ...
                    callback(err.code === 'ENOENT' ? Object.assign(Object.assign({}, err), { statusCode: 404 }) : err, search);
                }
            });
        }
    }
    ...
    upload(search, options, callback) {
        ...
        if (options && options.tags) {
            if (!Array.isArray(options.tags)) {
                return callback(new Error('Tags must be specified as an array; ' +
                    typeof options.tags +
                    ' provided'));
            }
        }
