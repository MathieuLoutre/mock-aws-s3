...
S3Mock.prototype = {
  ...
  putObjectAcl: function(args, callback) {
    callback(undefined, {});
  },
  ...
  createPresignedPost(params, callback) {
    if (typeof params.Bucket != 'string') {
      callback(new Error("Expected params.Bucket to be a string"));
    }
    else if (typeof params.Fields != 'object') {
      callback(new Error("Expected params.Fields to be an object"));
    }
    else if (typeof params.Fields.key != 'string') {
      callback(new Error("Expected params.Fields.key to be a string"));
    }
    else {
      callback(null, {
        url: 'https://s3.us-east-2.amazonaws.com/' + params.Bucket + '/' + params.Fields.key,
        fields: {}
      });
    }
  }
...
exports.ElasticTranscoder = function () {
  const jobs = {};

  this.readPipeline = function(args, callback) {
    if (!args.Id) {
      callback("Missing Id property in config");
    } else {
      // XXX accept any pipeline id
      callback(undefined, {
        Pipeline: {
          InputBucket: 'myinput_bucket',
          OutputBucket: 'myoutput_bucket',
        }
      });
    }
  }

  this.createJob = function(args, callback) {
    if (!(args.Input && args.Outputs && args.PipelineId)) {
      callback('Mandatory properties missing in config');
    } else {
      const Id = Math.random().toString();
      jobs[Id] = true;
      callback(undefined, { Job: { Id } });
    }
  }

  this.readJob = function(args, callback) {
    if (!args.Id) {
      callback('Mandatory property "Id" missing in config');
    } else if (jobs[args.Id] === undefined) {
      callback('Job not found');
    } else {
      callback(undefined, { Job: { Status: 'Complete' } });
    }
  }
}
