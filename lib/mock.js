...
const error_1 = require("aws-sdk/lib/error");
...
class S3Mock {
    ...
    deleteObjects(search, callback) {
        ...
        if (errors.length > 0) {
            callback(new error_1.AWSError('Error deleting objects'), {
                // @ts-expect-error
                Errors: errors,
                // @ts-expect-error
                Deleted: deleted
            ...
            fs_extra_1.default.readFile(search.Bucket + '/' + search.Key, function (err, data) {
                if (!err) {
                    ...
                    callback(err.code === 'ENOENT'
                        ? new error_1.AWSError(JSON.stringify(Object.assign(Object.assign({}, err), { statusCode: 404 })))
                        : new error_1.AWSError(err.message), search);
                }
            });
        }
    }
    copyObject(search, callback) {
        ...
        fs_extra_1.default.copy(decodeURIComponent(search.CopySource), search.Bucket + '/' + search.Key, function (err) {
            callback(new error_1.AWSError(err.message), search);
        });
    }
        ...
        if (!callback) {
            ...
            fs_extra_1.default.readFile(path, function (err, data) {
                if (!err) {
                    ...
                    if (err.code === 'ENOENT') {
                        return callback(new error_1.AWSError(JSON.stringify({
                            cfId: undefined,
                            code: 'NoSuchKey',
    ...
    createBucket(params, callback) {
        ...
        if (err === null) {
            ...
            fs_extra_1.default.mkdirs(bucketPath, function (err) {
                return callback(new error_1.AWSError(err.message), {});
            });
        }
        else {
    ...
    deleteBucket(params, callback) {
        ...
        var opts = underscore_1.default.extend({}, this.defaultOptions, applyBasePath(params, this.config));
        if (err !== null) {
            callback(new error_1.AWSError(err.message), {});
        }
        var bucketPath = opts.basePath || '';
        bucketPath += opts.Bucket;
    ...
    putObjectTagging(search, callback) {
        ...
        if (!search.Tagging || !search.Tagging.TagSet) {
            return callback(new error_1.AWSError('Tagging.TagSet required'), {});
        }
    ...
    upload(search, options, callback) {
        ...
        if (options && options.tags) {
            if (!Array.isArray(options.tags)) {
                return callback(new error_1.AWSError(`Tags must be specified as an array; ${typeof options.tags} provided`), null);
            }
        }
